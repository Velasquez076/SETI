...
public class FranchiseHandler {
  ...
  public Mono<ServerResponse> saveFranchise(ServerRequest serverRequest) {
    ...
    return serverRequest.bodyToMono(FranchiseDto.class)
        ...
        .map(franchise -> buildWrapper(franchise, 201))
  ...
  public Mono<ServerResponse> updateFranchise(ServerRequest serverRequest) {
    return serverRequest.bodyToMono(FranchiseDto.class)
        ...
        .map(franchise -> buildWrapper(franchise, 200))
        .flatMap(dto -> ServerResponse.ok().bodyValue(dto))
        .doOnSuccess(sr -> log.info("Franchise updated successfully! {}", sr.statusCode()));
  }

  private WrapperResponse<FranchiseDto> buildWrapper(Franchise franchise, int code) {
    return new WrapperResponse<>(UUID.randomUUID(), HttpStatusCode.valueOf(code),
        new FranchiseDto(franchise.getId(), franchise.getName()));
  }
}
